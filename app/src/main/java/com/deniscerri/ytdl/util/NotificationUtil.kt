    ...
    fun createDownloadFinished(
        ...
        if (filepath != null){
            ...
            try{
                ...
                if (preferences.getBoolean("whatsapp_enabled", false) && WhatsAppUtil.isWhatsAppInstalled(context) && uris.isNotEmpty()) {
                    val whatsappIntent = Intent().apply {
                        action = "com.deniscerri.ytdl.SEND_WHATSAPP"
                        putExtra("file_uri", uris.first().toString())
                    ...
                    notificationBuilder.addAction(R.drawable.ic_whatsapp, res.getString(R.string.send_to_whatsapp), whatsappPendingIntent)
                }
            }catch (_: Exception){}
        }
