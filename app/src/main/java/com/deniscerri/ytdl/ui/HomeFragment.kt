...
class HomeFragment : Fragment(), HomeAdapter.OnItemClickListener, SearchSuggestionsAdapter.OnItemClickListener, OnClickListener {
    ...
    private var quickDownloadFab: com.google.android.material.floatingactionbutton.ExtendedFloatingActionButton? = null
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        quickDownloadFab = homeFabs!!.findViewById(R.id.quick_download_fab)
        ...
        val primaryColor = MaterialColors.getColor(swipeRefresh, R.attr.colorPrimary)
    ...
    override fun onResume() {
        ...
        requireView().post {
            checkClipboard().apply {
                ...
                // Show quick download FAB when there are results
                quickDownloadFab?.isVisible = !resultViewModel.uiState.value.processing && resultViewModel.totalCount.value > 0
                quickDownloadFab?.setOnClickListener {
                    quickDownload()
                }
...
//            searchSuggestionsLinearLayout!!.isVisible = suggestions.isNotEmpty()
    ...
    private fun quickDownload() {
        lifecycleScope.launch(Dispatchers.IO) {
            val snapshot = homeAdapter.snapshot()
            if (snapshot.isNotEmpty()) {
                val downloadItems = mutableListOf<com.deniscerri.ytdl.database.models.DownloadItem>()
                for (result in snapshot) {
                    val downloadItem = downloadViewModel.createDownloadItemFromResult(
                        result = result!!,
                        givenType = DownloadType.valueOf(sharedPreferences?.getString("preferred_download_type", "video") ?: "video")
                    )
